...
from handlers.auth import services
from keynodes import KeynodeSysIdentifiers, Keynodes
from sctp.calendar_getter import CalendarGetter
...
class CreateEvent(base.BaseHandler, tornado.auth.GoogleOAuth2Mixin):
    ...
    # @tornado.web.asynchronous
    def post(self):
        CalendarGetter().write()
        jsonFromRequest = "[{\"name\": \"%s\", \"start_date\": \"%s\", \"end_date\": \"%s\"}]" % (
        self.get_argument('name'), self.get_argument('startDate'), self.get_argument('endDate'))
